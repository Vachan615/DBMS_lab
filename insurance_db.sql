...
from accident a 
where exists  
(select * from person p, participated ptd where p.driver_id=ptd.driver_id and 
p.driver_name="Smith" and a.report_no=ptd.report_no); 
+-----------------------------+ 
...
(select car.reg_no from person p, owns o where p.driver_id=o.driver_id and 
o.reg_no=car.reg_no and p.driver_name="Smith"); 
select * from car; 
+---------------+--------+--------+ 
| reg_no        | model  | c_year | 
+---------------+--------+--------+ 
...
| KA-21-AC-5473 | Alto   |   2015 | 
| KA-21-BD-4728 | Triber |   2019 | 
+---------------+--------+--------+ -- Update the damage amount for the car with reg_no of KA-09-MA-1234 in the accident with 
report_no 65738 
update participated set damage_amount=10000 where report_no=65738 and reg_no="KA-09
MA-1234"; 
select * from participated; 
+-----------+---------------+-----------+---------------+ 
| driver_id | reg_no        | report_no | damage_amount | 
...
DELIMITER ; // 
INSERT INTO participated VALUES 
("D333", "KA-20-AB-4223", 66666, 20000); 
ERROR 1644 (45000): Driver has already participated in 2 
accidents 
+-----------+---------------+-----------+---------------+ 
| driver_id | reg_no        | report_no | damage_amount | 
+-----------+---------------+-----------+---------------+ 
